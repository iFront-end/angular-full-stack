...
export class CategoriesFormComponent implements OnInit {
  ...
  deleteCategory() {
    const decision = window.confirm(`Вы уверены, что хотите удалить категорию ${this.category.name}? При удалении категории, удаляться все позиции.`);

    if (decision) {
      this.categoriesService.delete(this.category._id)
        .subscribe(
          (responsive: Message) => MaterialService.toast(responsive.message),
          error => MaterialService.toast(error.error.message),
